...
auth.post('/token', async (c) => {
  ...
  // H6 FIX: Never use hardcoded fallback — require proper secret
  const jwtSecret = c.env.JWT_SECRET || c.env.API_KEY || c.env.ECHO_API_KEY;
  if (!jwtSecret) {
    return c.json(error('JWT signing unavailable — no secret configured', 'ECHO_CONFIG_ERROR', v(c)), 503);
  }
...
auth.post('/billing/webhook', async (c) => {
  ...
  // H5 FIX: PayPal webhook — verify via PayPal Notifications API
  const rawBody = await c.req.text();

  // Verify PayPal webhook signature
  const paypalWebhookId = c.env.PAYPAL_WEBHOOK_ID;
  if (!paypalWebhookId) {
    console.error(JSON.stringify({ ts: new Date().toISOString(), level: 'error', component: 'billing', message: 'PAYPAL_WEBHOOK_ID not configured' }));
    return c.json(error('PayPal webhook processing unavailable', 'ECHO_CONFIG_ERROR', v(c)), 503);
  }
  ...
  if (!transmissionId || !transmissionTime || !transmissionSig) {
    return c.json(error('Missing PayPal webhook signature headers', 'ECHO_AUTH_ERROR', v(c)), 401);
  }

  // Verify via PayPal Notifications API (server-side verification)
  try {
    const paypalVerifyResp = await c.env.ECHO_PAYPAL?.fetch?.('https://echo-paypal/webhooks/verify', {
      method: 'POST',
    ...
    if (paypalVerifyResp && !paypalVerifyResp.ok) {
      console.error(JSON.stringify({ ts: new Date().toISOString(), level: 'error', component: 'billing', message: 'PayPal webhook verification failed' }));
      return c.json(error('PayPal webhook verification failed', 'ECHO_AUTH_ERROR', v(c)), 401);
    }
  } catch {
    // If PayPal worker unavailable, reject — never process unverified webhooks
    return c.json(error('PayPal webhook verification unavailable', 'ECHO_CONFIG_ERROR', v(c)), 503);
  }
  ...
  if (event.event_type === 'CHECKOUT.ORDER.APPROVED' || event.event_type === 'PAYMENT.CAPTURE.COMPLETED') {
    ...
    if (tenantId && plan) {
      ...
      console.log(JSON.stringify({
        ts: new Date().toISOString(), level: 'info', component: 'billing',
        event: 'plan_upgraded', tenant_id: tenantId, plan, provider: 'paypal',
      }));
    }
  }
