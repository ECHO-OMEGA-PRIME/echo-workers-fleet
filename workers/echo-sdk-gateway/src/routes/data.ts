...
// M1 FIX: Hardened SQL safety — catches stacked queries, CHAR() tricks, string concat injection
function isSafeSQL(sql: string): boolean {
  const trimmed = sql.trim();
  const lower = trimmed.toLowerCase();
  // Must start with SELECT or WITH (for CTEs)
  if (!lower.startsWith('select') && !lower.startsWith('with')) return false;
  // Block dangerous keywords (DML, DDL, admin, injection vectors)
  const dangerous =
    /\b(insert|update|delete|drop|alter|create|replace|truncate|attach|detach|pragma|grant|revoke|exec|execute|xp_|sp_|call|load_extension|randomblob|writefile|fts3tokenize|char\s*\(|substr\s*\(.*,.*,|x'[0-9a-f]|sqlite_compileoption|typeof\s*\(|zeroblob|unicode|printf|group_concat|total_changes|last_insert_rowid|changes\s*\()\b/i;
  if (dangerous.test(trimmed)) return false;
  // Block UNION (even SELECT..UNION SELECT is a classic injection vector)
  if (/\bunion\b/i.test(trimmed)) return false;
  // Block comment-based injection vectors
  if (trimmed.includes('--') || trimmed.includes('/*') || trimmed.includes('*/')) return false;
  // Block semicolons (multi-statement injection)
  if (trimmed.includes(';')) return false;
  // Block hex escaping tricks
  if (/0x[0-9a-fA-F]+/.test(trimmed)) return false;
  // Block string concatenation tricks (||)
  if (trimmed.includes('||')) return false;
  // Block subqueries that could contain writes (nested parens with dangerous words)
  if (/\(\s*select\b.*\bfrom\b.*\bwhere\b.*\blike\b.*\bor\b/i.test(trimmed)) return false;
  // Max length guard — no legitimate query should be 10KB
...
data.get('/:domain/tables', async (c) => {
  ...
  try {
    ...
    // M3 FIX: Don't leak internal error details
    return c.json(
      error(
        'Failed to introspect tables',
        'ECHO_DB_ERROR',
        version,
      ),
      500,
...
data.post('/:domain/query', async (c) => {
  ...
  try {
    ...
    // M3 FIX: Sanitize — return safe message for SQL errors, no internal table/column names
    const safeMsg = isSyntaxError
