...
function createPassthrough(
  ...
  router.all('/*', async (c) => {
    ...
    try {
      // M2 FIX: Build clean headers — never forward the caller's auth headers upstream.
      // Only inject the gateway's own internal key for service-to-service auth.
      const fetchOpts: RequestInit = {
        method: c.req.method,
        headers: {
          'Content-Type': c.req.header('content-type') || 'application/json',
          'X-Echo-Internal-Key': c.env.API_KEY || c.env.ECHO_API_KEY || '',
          'X-Forwarded-By': 'echo-sdk-gateway',
          'X-Tenant-Id': (c.get as any)('auth_tenant_id') || 'unknown',
        },
      };
      ...
      const latency_ms = Date.now() - start;
      // M3 FIX: Don't leak internal error details to clients
      console.error(JSON.stringify({ ts: new Date().toISOString(), level: 'error', component: 'proxy', worker: workerName, error: String(e).slice(0, 300) }));
      return c.json(
        error(`Proxy to upstream service failed`, 'ECHO_PROXY_ERROR', version, latency_ms),
        502,
      );
    }
  });
