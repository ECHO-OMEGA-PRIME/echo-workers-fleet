--cf38cca5436207b9b011dcaa5a33818d3eee1632ea9529ae6f14d1554317
Content-Disposition: form-data; name="index.js"

var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });

// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/_internal/utils.mjs
// @__NO_SIDE_EFFECTS__
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/perf_hooks/performance.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/polyfill/performance.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/console.mjs
import { Writable } from "node:stream";

// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/mock/noop.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/console.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
globalThis.console = console_default;

// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/hrtime.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/process.mjs
import { EventEmitter } from "node:events";

// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/read-stream.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/tty/write-stream.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/node/internal/process/node-version.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/@cloudflare/unenv-preset/dist/runtime/node/process.mjs
...
// S:/cache/npm/_npx/32026684e21afda6/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
var VERSION = "1.0.0";
var WORKER_NAME = "echo-prime-relay";
...
async function getStats(kv) {
  const raw2 = await kv.get("relay:stats", "json");
  if (raw2) return raw2;
  return {
    events_relayed: 0,
    events_failed: 0,
...
async function saveStats(kv, stats) {
  stats.uptime_seconds = Math.floor((Date.now() - START_TIME) / 1e3);
  await kv.put("relay:stats", JSON.stringify(stats), { expirationTtl: 86400 * 30 });
}
__name(saveStats, "saveStats");
async function getSubscriptions(kv) {
  const raw2 = await kv.get("relay:subscriptions", "json");
  return raw2 || [];
}
__name(getSubscriptions, "getSubscriptions");
async function saveSubscriptions(kv, subs) {
  await kv.put("relay:subscriptions", JSON.stringify(subs));
}
__name(saveSubscriptions, "saveSubscriptions");
async function getEventLog(kv, limit = 50) {
  const raw2 = await kv.get("relay:event_log", "json");
  const events = raw2 || [];
  return events.slice(-limit);
}
__name(getEventLog, "getEventLog");
async function appendEventLog(kv, event) {
  const events = await getEventLog(kv, 500);
  events.push(event);
  const trimmed = events.slice(-500);
  await kv.put("relay:event_log", JSON.stringify(trimmed), { expirationTtl: 86400 * 7 });
}
__name(appendEventLog, "appendEventLog");
function getServiceBinding(env2, bindingName) {
  const binding2 = SERVICE_MAP[bindingName];
  if (!binding2) return null;
  return env2[binding2] || null;
}
__name(getServiceBinding, "getServiceBinding");
async function deliverToService(env2, target, event) {
  const start = Date.now();
  const fetcher = getServiceBinding(env2, target);
  if (!fetcher) {
    return {
      target,
      status: "failed",
      latency_ms: Date.now() - start,
      error: `Unknown service binding: ${target}`
    };
  }
  try {
    let url;
    ...
    switch (target) {
      case "shared_brain":
        url = "https://brain/ingest";
        body = JSON.stringify({
          instance_id: WORKER_NAME,
          role: "system",
          content: `RELAY EVENT [${event.type}] from ${event.source}: ${JSON.stringify(event.payload).slice(0, 500)}`,
          importance: event.priority === "critical" ? 9 : event.priority === "high" ? 7 : 5,
          tags: ["relay", event.type, event.source]
        });
        break;
      case "swarm_brain":
        url = "https://swarm/moltbook/post";
        body = JSON.stringify({
          author_id: WORKER_NAME,
          author_name: "Echo Prime Relay",
          author_type: "system",
          content: `[RELAY] ${event.type}: ${JSON.stringify(event.payload).slice(0, 300)}`,
          mood: event.priority === "critical" ? "alert" : "building",
          tags: ["relay", event.type]
        });
        break;
      case "engine_runtime":
        url = "https://engine/query";
        body = JSON.stringify(event.payload);
        break;
      case "echo_chat":
        url = "https://chat/generate";
        body = JSON.stringify(event.payload);
        break;
      case "speak_cloud":
        url = "https://speak/tts";
    ...
    return { target, status: "delivered", latency_ms: Date.now() - start };
  } catch (err) {
    return {
      target,
      status: "failed",
...
__name(deliverToService, "deliverToService");
async function deliverToEndpoint(endpoint, event, env2) {
  const start = Date.now();
  try {
    const resp = await fetch(endpoint, {
      ...
      headers: {
        "Content-Type": "application/json",
        "X-Echo-API-Key": env2.ECHO_API_KEY || "",
        "X-Relay-Event-Id": event.id,
        "X-Relay-Event-Type": event.type
      },
      body: JSON.stringify(event)
    ...
    return { target: endpoint, status: "delivered", latency_ms: Date.now() - start };
  } catch (err) {
    return {
      target: endpoint,
      status: "failed",
...
__name(deliverToEndpoint, "deliverToEndpoint");
async function fanOut(env2, event, subscriptions) {
  const results = [];
  if (event.target) {
    const targets = Array.isArray(event.target) ? event.target : [event.target];
    const directPromises = targets.map(async (t) => {
      if (t.startsWith("http")) {
        return deliverToEndpoint(t, event, env2);
      }
      return deliverToService(env2, t, event);
    });
    const directResults = await Promise.allSettled(directPromises);
    for (const r of directResults) {
      if (r.status === "fulfilled") results.push(r.value);
      else results.push({ target: "unknown", status: "failed", latency_ms: 0, error: String(r.reason) });
    }
  }
  const matchingSubs = subscriptions.filter(
    (s) => s.active && s.event_types.includes(event.type)
  );
  if (matchingSubs.length > 0) {
    const subPromises = matchingSubs.map(async (sub) => {
      if (sub.service_binding) {
        return deliverToService(env2, sub.service_binding, event);
      }
      if (sub.endpoint) {
        return deliverToEndpoint(sub.endpoint, event, env2);
      }
      return {
        target: sub.subscriber,
        status: "skipped",
        latency_ms: 0,
        error: "No endpoint or service_binding configured"
      };
    });
...
async function addToDeadLetter(kv, event, error3) {
  const dlq = await kv.get("relay:dead_letter", "json") || [];
  dlq.push({ ...event, payload: { ...event.payload, _dlq_error: error3, _dlq_at: (/* @__PURE__ */ new Date()).toISOString() } });
  const trimmed = dlq.slice(-200);
  await kv.put("relay:dead_letter", JSON.stringify(trimmed), { expirationTtl: 86400 * 14 });
}
__name(addToDeadLetter, "addToDeadLetter");
async function getDeadLetter(kv) {
  const raw2 = await kv.get("relay:dead_letter", "json");
  return raw2 || [];
}
__name(getDeadLetter, "getDeadLetter");
var app = new Hono2();
app.use("*", cors({ origin: /* @__PURE__ */ __name((o) => ALLOWED_ORIGINS.includes(o) ? o : ALLOWED_ORIGINS[0], "origin"), allowMethods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"] }));
app.use("*", async (c, next) => {
  await next();
...
app.get("/health", async (c) => {
  ...
  return c.json({
    ...
    version: VERSION,
    worker: WORKER_NAME,
    uptime_seconds: Math.floor((Date.now() - START_TIME) / 1e3),
...
app.post("/relay", async (c) => {
  ...
  log3("info", "Relaying event", { event_id: event.id, type: event.type, source: event.source, priority: event.priority });
...
app.post("/relay/batch", async (c) => {
  ...
  const subscriptions = await getSubscriptions(c.env.RELAY_KV);
  const batchResults = [];
  for (const raw2 of body.events) {
    if (!raw2.type || !raw2.source || !raw2.payload) continue;
    const event = {
      id: raw2.id || generateId(),
      type: raw2.type,
      source: raw2.source,
      target: raw2.target,
      payload: raw2.payload,
      priority: raw2.priority || "normal",
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      ttl: raw2.ttl
    };
    const results = await fanOut(c.env, event, subscriptions);
  ...
  stats.last_event_at = (/* @__PURE__ */ new Date()).toISOString();
  await saveStats(c.env.RELAY_KV, stats);
...
app.post("/relay/broadcast", async (c) => {
  ...
  log3("info", "Broadcasting event to all services", { event_id: event.id, type: event.type });
  const subscriptions = await getSubscriptions(c.env.RELAY_KV);
  const results = await fanOut(c.env, event, subscriptions);
  await appendEventLog(c.env.RELAY_KV, event);
  return c.json({
...
app.post("/subscriptions", async (c) => {
  ...
  if (body.service_binding && !SERVICE_MAP[body.service_binding]) {
    return c.json({ error: `Unknown service binding: ${body.service_binding}. Available: ${Object.keys(SERVICE_MAP).join(", ")}` }, 400);
  }
  const subs = await getSubscriptions(c.env.RELAY_KV);
  const existing = subs.find(
    (s) => s.subscriber === body.subscriber && s.active
  );
  if (existing) {
    ...
    log3("info", "Updated subscription", { subscriber: body.subscriber });
  ...
  subs.push(sub);
  await saveSubscriptions(c.env.RELAY_KV, subs);
  log3("info", "Created subscription", { sub_id: sub.id, subscriber: sub.subscriber, event_types: sub.event_types });
...
app.delete("/subscriptions/:id", async (c) => {
  ...
  await saveSubscriptions(c.env.RELAY_KV, subs);
  log3("info", "Deactivated subscription", { sub_id: id });
...
app.post("/dead-letter/retry", async (c) => {
  ...
  log3("info", "Retried dead letter queue", { retried, succeeded, remaining: remaining.length });
...
app.post("/dead-letter/purge", async (c) => {
  ...
  log3("info", "Purged dead letter queue");
  return c.json({ success: true, message: "Dead letter queue purged" });
});
app.get("/services", (c) => {
  return c.json({
    services: Object.entries(SERVICE_MAP).map(([name, binding2]) => ({
      name,
      binding: binding2,
      available: true
    })),
...
app.get("/event-types", (c) => {
  return c.json({ event_types: EVENT_TYPES });
});
...
app.post("/alert", async (c) => {
  ...
  const results = await fanOut(c.env, event, subscriptions);
  await appendEventLog(c.env.RELAY_KV, event);
  log3("warn", "Alert dispatched", { title: body.title, severity: body.severity });
  return c.json({
    event_id: event.id,
...
app.post("/engine/query", async (c) => {
  ...
  try {
    ...
    log3("error", "Engine query failed", { error: err instanceof Error ? err.message : String(err) });
...
app.post("/chat/generate", async (c) => {
  ...
  try {
    ...
    log3("error", "Chat generation failed", { error: err instanceof Error ? err.message : String(err) });
    return c.json({ error: "Chat generation failed" }, 502);
  }
});
app.notFound((c) => c.json({ error: "Not found", worker: WORKER_NAME }, 404));
app.onError((err, c) => {
  log3("error", "Unhandled error", { error: err.message, stack: err.stack });
